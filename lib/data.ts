...
const normalizeCityRecord = (record: Record<string, unknown>): City | null => {
  ...
  return {
    ...
    slug,
    name: (record.name as string | null | undefined) ?? fallbackName,
    hero_title:
      (record.hero_title as string | null | undefined) ??
      fallback?.hero_title ??
      `Exterior remodeling in ${fallbackName}`,
    hero_description:
      (record.hero_description as string | null | undefined) ?? fallback?.hero_description ?? null,
    service_highlights:
      (record.service_highlights as string[] | null | undefined) ?? fallback?.service_highlights ?? null,
    faqs:
      (record.faqs as { question: string; answer: string }[] | null | undefined) ?? fallback?.faqs ?? null,
    updated_at: (record.updated_at as string | null | undefined) ?? fallback?.updated_at ?? null
  };
};
...
const normalizePostRecord = (record: Record<string, unknown>): Post | null => {
  ...
  const title = (record.title as string | null | undefined) ?? fallback?.title ?? toTitleFromSlug(slug);
  const publishedAt =
    (record.published_at as string | null | undefined) ?? fallback?.published_at ?? null;
  const updatedAt =
    (record.updated_at as string | null | undefined) ?? fallback?.updated_at ?? publishedAt ?? null;

  return {
    id: (record.id as string | number | undefined)?.toString() || fallback?.id || slug,
    slug,
    title,
    excerpt: (record.excerpt as string | null | undefined) ?? fallback?.excerpt ?? null,
    content: (record.content as string | null | undefined) ?? fallback?.content ?? null,
    featured_image:
      (record.featured_image as string | null | undefined) ?? fallback?.featured_image ?? null,
    city_slug: (record.city_slug as string | null | undefined) ?? fallback?.city_slug ?? null,
