(function () {
  ...
  const trackEvent = (detail) => {
    ...
    if (typeof window.gtag === 'function') {
      window.gtag('event', detail.interaction_type || 'interaction', {
        event_category: detail.event_category || 'engagement',
        event_label: detail.label || detail.location || 'interaction',
        location: detail.location || 'unspecified',
        lead_city: detail.city || 'unspecified'
      });
    }
  };
  ...
  document.querySelectorAll('[data-track="cta"]').forEach((el) => {
    el.addEventListener('click', () => {
      trackEvent({
        ...
        city: el.dataset.city || 'unspecified'
      });
    });
  });
  ...
  document.querySelectorAll('form[data-provider="supabase"]').forEach((form) => {
    ...
    form.addEventListener('submit', async (event) => {
      ...
      trackEvent({
        ...
        city: formData.city || form.dataset.city || 'unspecified'
      });
  ...
  document.querySelectorAll('[data-source]').forEach(async (section) => {
    const source = section.getAttribute('data-source');
    const target = section.querySelector('.post-grid');
    if (!source || !target) {
      return;
    }
    try {
      ...
      target.innerHTML = posts
        ...
        .map(
          (post) => `
            <article>
              <h3><a href="${post.url}" data-track="cta" data-location="blog-feed" data-city="${post.city || 'unspecified'}">${post.title}</a></h3>
              <p>${post.excerpt || ''}</p>
              <p class="post-meta">City: ${post.city || 'Seattle Metro'} &bull; Tags: ${(post.tags || []).join(', ')}</p>
            </article>
          `
        )
        .join('');
    } catch (error) {
      // Fail silently if feed cannot be loaded
    }
  });
